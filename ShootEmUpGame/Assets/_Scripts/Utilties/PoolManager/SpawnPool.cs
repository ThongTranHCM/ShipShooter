...
namespace GamePoolManager
{
    [AddComponentMenu("GamePoolManager/SpawnPool")]
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
		...
		// --- @author: THAITQ ---
		#region CreatePrefabPool (THAITQ)
		public Coroutine RoutineCreatePrefabPool(PrefabPool prefabPool, int _numPrefabCreatePerTime = 1, float _timeDelayToNextSpawnNewPrefab = 0f){ // tham khảo hàm CreatePrefabPool(PrefabPool prefabPool)
			bool isAlreadyPool = this.GetPrefabPool(prefabPool.prefab) == null ? false : true;
			if (isAlreadyPool){
			...
			if (!prefabPool.preloaded){
				...
				return StartCoroutine(DoActionCreatePrefabPool(prefabPool, _numPrefabCreatePerTime, _timeDelayToNextSpawnNewPrefab));
			}else{
				return null;
			}
		}

		IEnumerator DoActionCreatePrefabPool(PrefabPool prefabPool, int _numPrefabCreatePerTime, float _timeDelayToNextSpawn){
			...
			if (prefabPool.limitInstances && prefabPool.preloadAmount > prefabPool.limitAmount)
			{
				Debug.LogWarning
				(
					string.Format
				...
				prefabPool.preloadAmount = prefabPool.limitAmount;
			}

			// Notify the user if they made a mistake using Culling
			//   (First check is cheap)
			if (prefabPool.cullDespawned && prefabPool.preloadAmount > prefabPool.cullAbove)
			{
			...
			if (prefabPool.preloadTime)
			{
				if (prefabPool.preloadFrames > prefabPool.preloadAmount)
				{
					Debug.LogWarning(string.Format("SpawnPool {0} ({1}): " +
					...
					prefabPool.preloadFrames = prefabPool.preloadAmount;
				}

				yield return StartCoroutine(DoAtionLoadNewPrefabPool(prefabPool, _numPrefabCreatePerTime, _timeDelayToNextSpawn));
			}
			else
			{
				...
				Transform inst;
				while (prefabPool.totalCount < prefabPool.preloadAmount) // Total count will update
				{
					// Preload...
		...
		IEnumerator DoAtionLoadNewPrefabPool(PrefabPool _prefabPool, int _numPrefabCreatePerTime, float _timeDelayToNextSpawn)
		{
//			yield return new WaitForSeconds(_prefabPool.preloadDelay);

			Transform inst;

			// subtract anything spawned by other scripts, just in case
			int amount = _prefabPool.preloadAmount - _prefabPool.totalCount;
			if (amount <= 0)
				yield break;

			// Reduce debug spam: Turn off this.logMessages then set it back when done.
			_prefabPool.forceLoggingSilent = true;

			int numThisFrame;
			WaitForSeconds timeDelay = new WaitForSeconds (_timeDelayToNextSpawn);
			for (int i = 0; i < _prefabPool.preloadAmount; i++){
				for(int j = 0; j < _numPrefabCreatePerTime; j++){
					inst = _prefabPool.SpawnNew();
					if (inst != null){
						_prefabPool.DespawnInstance(inst, false);
					}
					if (_prefabPool.totalCount > _prefabPool.preloadAmount)
						break;
				}
				if (_prefabPool.totalCount > _prefabPool.preloadAmount){
					break;
...
//				}
			...
			// Restore the previous setting
			_prefabPool.forceLoggingSilent = false;
		}
		#endregion

		public int GetNumItemInPrefabPool(PrefabPool _prefabPool){
			return ((int) (_prefabPool._spawned.Count + _prefabPool._despawned.Count));
		}

		public void Remove(Transform item)
		{
			for (int i = 0; i < this._prefabPools.Count; i++){
				if(this._prefabPools[i].Contains(item)){
					Remove(item, this._prefabPools[i]);
					break;
				}
			}
		}
		public void Remove(Transform _item, PrefabPool _prefabPoolOfItem){
			if(_prefabPoolOfItem.Contains(_item)){
				if(_prefabPoolOfItem._spawned.Contains(_item)){
					_prefabPoolOfItem._spawned.Remove(_item);
				}else if(_prefabPoolOfItem._despawned.Contains(_item)){
					_prefabPoolOfItem._despawned.Remove(_item);
				}
				_prefabPoolOfItem.spawnPool.DestroyInstance(_item.gameObject);
			}
		}

		public void Remove(PrefabPool _prefabPool){
			this.prefabs._Remove(_prefabPool.prefab.name);
			_prefabPool.SelfDestruct();
			this._prefabPools.Remove(_prefabPool);
		}

		public Coroutine RemoveAllPrefabPoolExceptList(List<PrefabPool> _listPrefabPoolExcept){
			return StartCoroutine(DoActionRemoveAllPrefabPoolExceptList(_listPrefabPoolExcept));
		}

		IEnumerator DoActionRemoveAllPrefabPoolExceptList(List<PrefabPool> _listPrefabPoolExcept){
			for (int i = 0; i < this._prefabPools.Count; i++){
				if(!_listPrefabPoolExcept.Contains(this._prefabPools[i])){
					Remove(this._prefabPools[i]);
				}
			}
			yield return null;
		}

		public Coroutine RemoveAllPrefabPool(){
			return StartCoroutine(DoActionRemoveAllPrefabPool());
		}

		IEnumerator DoActionRemoveAllPrefabPool(){
			for (int i = 0; i < this._prefabPools.Count; i++){
        ...
        public Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot, Transform parent)
        {
			...
			#if UNITY_EDITOR
			// Debug.LogError("CREATE NEW: " + inst.gameObject.name + " -- PARENT: " + inst.transform.parent.name);
			#endif
        ...
        /// 
        /// IMPORTANT: 
        ///     * You mustdsition and rotation. The return Type is different though.
        /// </summary>
        ...
        public void ReparentToGroup(Transform instance)
        {
...
//                yield return null;
    ...
    public class PrefabPool
    {
		...
		public bool forceLoggingSilent {get;set;}
        ...
        internal IEnumerator CullDespawned()
        {
			...
			WaitForSeconds delayCull = new WaitForSeconds(this.cullDelay);
			yield return delayCull;
		...
		public IEnumerator PreloadOverTime()
        {
