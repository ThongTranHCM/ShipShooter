...
public class DailyOfferContentManager : MonoBehaviour
{
    ...
    public void UpdateOfferPanel(List<DailyOfferManager.Offer> offerList, int Index){
        ...
        foreach(Transform child in gameObject.transform){
            ...
            if(i > Index){
                child.gameObject.GetComponent<OfferLidManager>().SetLock();
            }
            else if (i < Index){
                child.gameObject.GetComponent<OfferLidManager>().SetClaimed();
            }
            else{
                child.gameObject.GetComponent<OfferLidManager>().SetUnlock();
            }
            i += 1;
        }
    }
